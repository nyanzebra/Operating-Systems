...
thread_create(const char *name)
{
	...
	// them here.
	array_preallocate(&thread->t_openfiletable,MAXFILETABLESIZE); 
	    ...
	    struct thread **ret)
{
	...
	/* Get a PID */
	result = allocatePID(&newguy->t_pid);
	if (result) {
		goto fail;
	}
  
  /* 
	 Check if thread is passed return value for childpid. If we do not use childpid, skip
	 copying filetable and address space.
	 */
  ...
  if (ret != NULL) {
		...
		if (curthread->t_openfiletable != NULL) {
			result = array_preallocate(&newguy->t_openfiletable, array_getnum(&newguy->t_openfiletable));
			if (result) {
				goto fail1;
			}
		}
		...
		// Set return to new process's pid
		*ret = newguy->t_pid;
	}
...
fail2:
	if (newguy->t_openfiletable) {
		array_destroy(newguy->t_openfiletable);
		newguy->t_openfiletable = NULL;
	}
 
fail1:
	if (newguy->t_pid != INVALID_PID) {
		unallocatePID(newguy->t_pid);
		newguy->t_pid = INVALID_PID;
	}
...
thread_exit(int exitstatus)
{
	...
	// Project 4: Set the exit status
	setPIDexitStatus(exitstatus);
	...
	// Project 4 Addition: Clean up file table
	if (curthread->t_openfiletable) {
		array_destroy(curthread->t_openfiletable);
		curthread->t_openfiletable = NULL;
	}
